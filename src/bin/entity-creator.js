...
export default(gameMap, level = 1) => {
  const bosses = [];
    if (level === 4) {
      bosses.push({health: 400, level: 5, type: 'boss',});
    }
    ...
    const weapons = [];
    const qualifying = weaponTypes.filter(
      weapon => weapon.damage < level * 10 + 10
