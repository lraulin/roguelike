...
export default () => {
  ...
  const createRoomsFromSeed = (grid, {x, y, width, height}, range = c.ROOM_SIZE_RANGE) => {
    // range for generating random heights and widths
    const [min, max] = range;

    // generate values for each edge of the seed room
    const roomValues = [];
  ...
  // recursively add rooms using first as seed
  const growMap(grid, seedRooms, counter = 1, maxRooms = c.MAX_ROOMS) => {
    if (count + seedRooms.length > maxRooms || !seedRooms.length) {
      return grid;
    }
