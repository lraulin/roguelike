...
function addWeapon(payload) {
  return {type: t.ADD_WEAPON, payload,};
}

function addXP(payload) {
  return {type: t.ADD_XP, payload,};
}
...
function changePlayerPosition(payload) {
  return {type: t.CHANGE_PLAYER_POSITION, payload,};
}
...
function modifyHealth(payload) {
  return {type: t.MODIFY_HEALTH, payload,};
}

function newMessage(payload) {
  return {type: t.NEW_MESSAGE, payload,};
}

export function restart() {
  return {type: t.RESTART};
}

export function setDungeonLevel(payload) {
  return {type: t.SET_DUNGEON_LEVEL, payload,};
}

export function toggleFogMode() {
  return {type: t.TOGGLE_FOG_MODE};
}
...
export default(vector) => {
  return(dispatch, getState) => {
    ...
    const [x, y,] = grid.playerPosition.slice(0); // current location
    const [vectorX, vectorY,] = vector; // directon modifier
    const newPosition = [
      vectorX + x,
      vectorY + y,
    ]; // target location
    const newPlayer = gird.entities[y][x];
    const destination = grid.entities[y + vectorY][x + vectorX]; // target cell contents
    const actions = [];
    ...
    switch (destination.type) {
      ...
      case 'enemy':
        {
          ...
          const enemyDamageTaken = Math.floor(
            player.weapon.damage * _.random(1, 1.3) * playerLevel
          );
          ...
          if (destination.health > 0) {
            ...
            if (player.health - playerDamageTaken <= 0) {
              ...
              setTimeout(
                () => dispatch(newMessage(`You resolve to try harder next time`)),
                4000
              );
              setTimeout(() => dispatch(newMessage(`The grid resets itself....`)), 6000);
              setTimeout(
                () => dispatch(batchActions([restart(), createLevel(1), setDungeonLevel(1),])),
                8000
              );
              return;
            }
          ...
          if (destination.health <= 0) {
            // the fight is over and the player has won add XP and move the player
            if (destination.type === 'boss') {
              ...
              setTimeout(
                () => dispatch(newMessage(`The BOSS emits an almighty scream`)),
                2000
              );
              setTimeout(
                () => dispatch(newMessage(`You bask momentarily in your glory`)),
                4000
              );
              setTimeout(() => dispatch(newMessage(`The grid resets itself....`)), 6000);
              setTimeout(
              ...
              setTimeout(
                () => dispatch(newMessage(`You gain 10XP and feel yourself growing stronger..`)),
                2500
              );
      ...
      case 'exit':
        ...
        actions.push(newMessage(
          `The cells start to shift... you transit to zone ${grid.dungeonLevel + 1}`
        ));
        setTimeout(
